...
object AbiquoAPI {
	...
	def reportUserLoop(s:Session) = {
		var userloop = LOG.error(
			"vappId {} \tvmpostRt {} \tdRetry {} \tuRetry {} \tdeployRetry {} \tundeployRetry {} \tunkDeploy {} \tunkUndeploy {} \tlastDeployTime {} \tlastUndeployTime {} ",
				Array[Object](
		...
		.removeAttribute("unknownDeployRetry").removeAttribute("unknownUndeployRetry")
	}

	def getUnknownUndeployRetry(s:Session) = {
		if(s.isAttributeDefined("unknownUndeployRetry")) { s.getTypedAttribute[Long]("unknownUndeployRetry") + 1 } else { 0l }
	}
	def getUnknownDeployRetry(s:Session) = {
		if(s.isAttributeDefined("unknownDeployRetry")) { s.getTypedAttribute[Long]("unknownDeployRetry") + 1 } else { 0l }
	}
